...
def get_interpreter():
    """
    Return the tflite.Interpreter singleton.
    Import tflite_runtime lazily so the module can be imported even if
    tflite_runtime is not installed (helpful during local dev with full TF).
    """
    global _interpreter
    ...
    try:
        # Render / production: tflite-runtime only
        import tflite_runtime.interpreter as tflite
        _interpreter = tflite.Interpreter(model_path=str(MODEL_CACHE_PATH))
    except ImportError:
        # Local dev fallback: full TensorFlow
